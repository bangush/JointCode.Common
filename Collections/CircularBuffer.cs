...
namespace JointCode.Common.Collections
{
    ...
    public class CircularBuffer<T> : ICollection<T>, ICollection
    {
        ...
        public T this[int index]
        {
            get
            {
                if (index >= Size) throw new InternalBufferOverflowException("The index cannot be greater than the buffer size.");
                if (Head + index < Capacity)
                    return _buffer[Head + index];
                else
                    return _buffer[(Head + index) - Capacity];
            }
        }
